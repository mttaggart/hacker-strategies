...
const program = new (_commander().default.Command)(); // Exit codes in response to signals are traditionally
// 128 + signal value
...
program.version(_package().version); // --no-cache, --cache-dir, --no-source-maps, --no-autoinstall, --global?, --public-url, --log-level
...
const commonOptions = {
  ...
  '--detailed-report [count]': ['print the asset timings and sizes in the build report', parseOptionInt, '10'],
...
function parsePort(portValue) {
  let parsedPort = Number(portValue); // Throw an error if port value is invalid...
...
async function normalizeOptions(command, inputFS) {
  ...
  if (command.name() === 'build') {
    nodeEnv = process.env.NODE_ENV || 'production'; // Autoinstall unless explicitly disabled or we detect a CI environment.
  ...
  let {
    host
  } = command; // Ensure port is valid and available
