...
class NodeResolver {
  ...
  async resolveModule({
    ...
    let sourceFile = parent || _path().default.join(this.projectRoot, 'index'); // If this isn't the entrypoint, resolve the input file to an absolute path


    if (parent) {
      filename = await this.resolveFilename(filename, _path().default.dirname(sourceFile), isURL);
    ...
    if (resolved === undefined) {
      ...
      let alternativeModules = await (0, _utils().findAlternativeNodeModules)(this.fs, resolved.moduleName, _path().default.dirname(sourceFile));

      if (alternativeModules.length) {
        throw new (_diagnostic().default)({
          diagnostic: {
            message: (0, _diagnostic().md)`Cannot find module ${(0, _nullthrows().default)(resolved).moduleName}`,
  ...
  findPackage(sourceFile, ctx) {
    ...
    if (pkgFile) {
      return this.readPackage(_path().default.dirname(pkgFile), ctx);
    }
