...
function _NodeResolver() {
  const data = require("./NodeResolver");

  _NodeResolver = function () {
    return data;
  };
...
function _NodeResolverSync() {
  const data = require("./NodeResolverSync");

  _NodeResolverSync = function () {
    return data;
  };
...
class NodePackageManager {
  constructor(fs, installer) {
    ...
    _defineProperty(this, "syncResolver", void 0);

    this.fs = fs;
    this.installer = installer;
    this.resolver = new (_NodeResolver().NodeResolver)(this.fs);
    this.syncResolver = new (_NodeResolverSync().NodeResolverSync)(this.fs);
  }
  ...
  async resolve(name, from, options) {
    ...
    if (!resolved) {
      try {
        ...
        let conflicts = await (0, _utils().getConflictingLocalDependencies)(this.fs, name, from);
      ...
      cache.set(key, resolved); // Add the specifier as a child to the parent module.
  ...
  async install(modules, from, opts) {
    await (0, _installPackage().installPackage)(this.fs, this, modules, from, {
