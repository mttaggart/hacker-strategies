...
function _validateModuleSpecifier() {
  const data = _interopRequireDefault(require("./validateModuleSpecifier"));

  _validateModuleSpecifier = function () {
    return data;
  };
...
async function install(fs, packageManager, modules, from, options = {}) {
  let {
    installPeers = true,
    saveDev = true,
    packageInstaller
  ...
  if (installPeers) {
    await Promise.all(modules.map(m => installPeerDependencies(fs, packageManager, m, from, options)));
  }
}

async function installPeerDependencies(fs, packageManager, module, from, options) {
  const {
    resolved
  ...
  if (modules.length) {
    await install(fs, packageManager, modules, from, Object.assign({}, options, {
      installPeers: false
    }));
  }
}
...
async function determinePackageInstaller(fs, filepath) {
  let configFile = await (0, _utils().resolveConfig)(fs, filepath, ['package-lock.json', 'pnpm-lock.yaml', 'yarn.lock']);
...
function _addToInstallQueue(fs, packageManager, modules, filePath, options) {
  ...
  if (modulesToInstall.length) {
    ...
    queue.add(() => install(fs, packageManager, modulesToInstall, filePath, options).then(() => {
      for (let m of modulesToInstall) {
...
function installPackage(fs, packageManager, modules, filePath, options) {
