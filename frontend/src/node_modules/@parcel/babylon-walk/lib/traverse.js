...
function _explode() {
  const data = _interopRequireDefault(require("./explode.js"));
...
class Path {
  constructor(node, parent, listkey, key) {
    ...
    _defineProperty(this, "_skipped", false);
...
function traverseWalk(visitors, state, node, parent, listkey, key) {
  if (!node || visitors.shouldSkip && visitors.shouldSkip(node) === true) return;
  const {
    enter,
    exit
