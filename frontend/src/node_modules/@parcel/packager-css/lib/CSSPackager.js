...
async function processCSSModule(options, logger, bundleGraph, bundle, asset, media) {
  ...
  if (usedSymbols.has('default')) {
    let incoming = bundleGraph.getIncomingDependencies(asset); // `import * as ns from ""; ns.default` is fine.
    ...
    if (defaultImport) {
      ...
      logger.warn({
        ...
        hints: [`Instead do: import * as style from "${defaultImport.moduleSpecifier}";`]
      });
    }
  }
